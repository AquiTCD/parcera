...
The settings system is a React-based UI that communicates with the Tauri Rust backend via IPC (`invoke`) to manage YAML-based configurations.
...
- **Syncing**: Saving settings triggers an `invoke` call to the Tauri backend, which updates the YAML file and notifies the Python sidecar via a POST request.
- **Tabs**:
