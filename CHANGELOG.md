...
## [0.14.10] - 2026-05-21 - Windows TTS Engine Path Fix

### Fixed
- **Windows**: AivisSpeech and VOICEVOX engine auto-start now uses the correct `%LOCALAPPDATA%\Programs\...\run.exe` default path instead of the macOS `/Applications/...` path

### Changed
- README rewritten to reflect current state: Tauri app distribution, macOS + Windows support, all provider options, correct OBS browser source URLs
